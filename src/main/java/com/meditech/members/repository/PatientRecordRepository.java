...
//    Integer findLatestTurnByPatientId(@Param("patientId") Long patientId);
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM PatientRecordEntity p WHERE p.id.patientEntity.id = :patientId AND p.id.turn = :turn")
    void deleteLatestRecordByPatientId(@Param("patientId") Long patientId, @Param("turn") int turn);
